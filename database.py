...
class DatabaseManager:
    ...
    def dodaj_studenta(self, student: Student):
        ...
        try:
            with self._connect() as conn:
                conn.execute(query, (
                    student.imie, student.nazwisko, student.adres,
                    student.nr_indeksu, student.pesel, student.plec
                ))
            return True, "Student dodany pomyślnie."
        except sqlite3.IntegrityError:
            return False, "Błąd: Student o takim numerze indeksu lub PESEL już istnieje."
        except Exception as e:
            return False, f"Wystąpił nieoczekiwany błąd bazy danych: {e}"

    def pobierz_wszystkich(self):
        
        query = "SELECT imie, nazwisko, adres, nr_indeksu, pesel, plec FROM studenci"
